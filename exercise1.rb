...
#Q Find the possible venues that are wheelchair accessible, in Toronto, and can fit at least 150 people. These results should be stored in an array.
arr=[]

venues.each do  |x|

if x[:wheelchair_accessible] && x[:capacity]>=100 && x[:city]=="Toronto"
  arr<<(x[:address])
  # arr<<venues
end
end
